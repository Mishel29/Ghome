...
export default function PublicNav() {
  const { user, logout, savedIds, nav, setShowLogin } = useApp();
    ...
    <>
      ...
      <nav className="bg-navy sticky top-0 z-40 shadow-md">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-14">
          {/* Logo */}
          <button onClick={() => nav("home")} className="flex items-center gap-2 text-white font-display font-bold text-xl tracking-tight hover:opacity-90 transition-opacity">
          ...
          <div className="hidden md:flex items-center gap-1">
            {[
              { label: "Find Your Home", action: () => nav("properties") },
              { label: "Mortgage Calculator", action: () => nav("mortgage") },
              { label: "Compare", action: () => nav("compare") },
              { label: "Analytics", action: () => nav("analytics") },
              { label: "News", action: () => nav("news") },
            ].map(({ label, action }) => (
              <button
                key={label}
                onClick={action}
          ...
          <div className="flex items-center gap-2">
            ...
            {user ? (
              <div className="flex items-center gap-3">
                ...
                <span className="text-white/70 text-sm hidden lg:block">{user.name}</span>
                <button onClick={logout} className="text-white/70 hover:text-white text-sm flex items-center gap-1 transition-colors">
        ...
        {mobileOpen && (
          <div className="md:hidden bg-navy-light border-t border-white/10">
            {[
              { label: "Find Your Home", action: () => nav("properties") },
              { label: "Mortgage Calculator", action: () => nav("mortgage") },
              { label: "Compare Properties", action: () => nav("compare") },
              { label: "Analytics", action: () => nav("analytics") },
              { label: "News", action: () => nav("news") },
              { label: "AI Chatbot", action: () => nav("chatbot") },
              ...(savedIds.length > 0 ? [{ label: `Saved (${savedIds.length})`, action: () => nav("saved") }] : []),
            ].map(({ label, action }) => (
              <button
                key={label}
