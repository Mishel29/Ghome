...
import { cancelCsv, getImport, getUpload, importHistory, resolvePropertyDuplicates, startCsv, uploadCsv, validateCsv } from "../../api/imports";
...
const propertyCsvColumns = ["Name", "Address", "Postal Code", "County", "Price", "Sold times", "Property Type", "Status", "Stage", "Agent", "Description", "Property Size Category", "Property Size", "Beds", "Baths", "Completion Year", "Years", "Historical Prices", "Image URL", "Video URL"];
export default function PropertyImport({ subscribers = false }: { subscribers?: boolean }) {
  ...
  const [duplicateActions,setDuplicateActions]=useState<Record<number,"SKIP"|"REPLACE">>({});
    ...
    {!sessionId && <>
      ...
      {upload && upload.id === uploadId && <section className="bg-white border border-[#ddd5c5] p-6 space-y-4">
        ...
        {report && <><div className="grid sm:grid-cols-3 gap-4">{[["Total rows",report.totalRows],["Valid rows",report.validRows],["Invalid rows",report.invalidRows]].map(([k,v]) => <div key={k} className="bg-cream p-4"><p className="text-xs text-stone">{k}</p><p className="text-2xl font-semibold">{v}</p></div>)}</div>
          {!subscribers&&report.duplicateProperties.length>0&&<section className="border border-amber bg-amber/10 p-4 space-y-3"><h3 className="font-semibold">Duplicate property names</h3><p className="text-sm">Database matches must be skipped or replaced. Replacements are saved as drafts. Duplicate names inside this CSV must be removed and uploaded again.</p><div className="space-y-2">{report.duplicateProperties.map((duplicate)=><div key={`${duplicate.source}-${duplicate.rowNumber}`} className="bg-white p-3 text-sm flex flex-wrap items-center gap-3"><span>Row {duplicate.rowNumber}: <strong>{duplicate.name}</strong>{duplicate.existingPropertyName&&` matches ${duplicate.existingPropertyName}`}</span>{duplicate.source==="DATABASE"?<select value={duplicateActions[duplicate.rowNumber]??duplicate.resolution??""} onChange={(e)=>setDuplicateActions({...duplicateActions,[duplicate.rowNumber]:e.target.value as "SKIP"|"REPLACE"})}><option value="">Choose action</option><option value="SKIP">Skip import row</option><option value="REPLACE">Replace database property as draft</option></select>:<span className="text-red-700">Remove duplicate CSV row</span>}</div>)}</div>{report.unresolvedDuplicateCount>0&&report.duplicateProperties.every((duplicate)=>duplicate.source==="DATABASE"?!!duplicateActions[duplicate.rowNumber]:false)&&<button className={button+" bg-navy text-white"} disabled={!!busy} onClick={()=>void act("Saving duplicate decisions",async()=>{const resolutions=report.duplicateProperties.filter((duplicate)=>duplicate.source==="DATABASE").map((duplicate)=>({rowNumber:duplicate.rowNumber,action:duplicateActions[duplicate.rowNumber]}));setUpload(await resolvePropertyDuplicates(upload.id,resolutions));})}>Apply duplicate decisions</button>}</section>}
