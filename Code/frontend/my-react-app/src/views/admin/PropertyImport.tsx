  ...
  return <div className="p-5 md:p-8 max-w-7xl space-y-6 text-navy">
    ...
    {!sessionId && <>
      ...
      {upload && upload.id === uploadId && <section className="bg-white border border-[#ddd5c5] p-6 space-y-4">
        ...
        {upload.status === "UPLOADED" && <button className={button} disabled={!!busy} onClick={() => void act("Validating all rows", async () => { setUpload(await validateCsv(upload.id)); })}>Validate File</button>}
