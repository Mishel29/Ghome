...
describe("campaign statistics aggregation", () => {
  it("keeps saves and interests in their campaign/day bucket", () => {
    const days = aggregateCampaignStatistics([
      ...
      event("OPENED", "2026-09-01T10:30:00.000Z"),
    ...
    expect(days).toEqual(expect.arrayContaining([
      expect.objectContaining({ campaignId: "campaign-a", sent: 1, opens: 1, clicks: 1, saves: 2, interests: 1, clickRate: 1, interestRate: 1 }),
      expect.objectContaining({ campaignId: "campaign-b", sent: 0, opens: 0, clicks: 0, saves: 1, interests: 0, clickRate: 0, interestRate: 0 }),
    ]));
  });

  it("returns explicit zero values for metrics with no events", () => {
    expect(aggregateCampaignStatistics([event("SENT", "2026-09-02T10:00:00.000Z")])[0]).toMatchObject({ opens: 0, clicks: 0, interests: 0, saves: 0, unsubscribes: 0 });
  });
